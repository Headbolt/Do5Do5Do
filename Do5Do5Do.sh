...
#
#	Do5Do5Do.sh
#	https://github.com/Headbolt/Do5Do5Do
#
...
elevate=${7} # Grab whether to run this Policy Trigger usinf SUDO from JAMF variable #8 eg YES
...
ScriptName="MacOS | Trigger - Pause 5 - Trigger - Pause 5 - Trigger"
#
...
#
Command=$(/bin/echo $Elevate /usr/local/bin/jamf policy -trigger '"'$Trigger1'"' '&&' sleep 5 '&&' $Elevate /usr/local/bin/jamf policy -trigger '"'$Trigger2'"' '&&' sleep 5 '&&' $Elevate /usr/local/bin/jamf policy -trigger '"'$Trigger3'"' $FB)
#
/bin/echo "$Command"
eval "$Command"
#
